...
def palindrome(s):
    left = 0
    right = len(s) - 1  # subtract one since you start at index 0 and that is already taken
    while left < right:
        if s[left] != s[right]:
            return False
        left += 1  # increases their respective counts in order for it to count to 0
        right -= 1  # decreases the value from the len of(s) in order to meet up with the left pointer
        # will continue to run the while statement and the if until the index do not meet up
    return True


def reverse_vowels(input_str):
    lst = list(input_str)  # guaranteed linear time
    left = 0
    right = len(lst) - 1
    vowels = "aeiou"
    while left < right:
        if lst[left] in vowels and lst[right] in vowels:  # check the index contains a vowel value
            lst[left], lst[right] = lst[right], lst[left]  # switches the values around
            left += 1
            right -= 1
        if lst[left] not in vowels:  # if the value in the index is not a vowel, increase the count like normal
            left += 1
        if lst[right] not in vowels:  # same here
            right -= 1
    return "".join(lst)
...
def find_missing(lst):
    ...
    if len(lst) == 0:  # edge case whenever the lst is empty and there are not provided values inside
        return 0

    if lst[
        right] == right:  # since the index of the lst is already reduced on the right side, if the index of the right
        # is equal to the value then that means the lst is missing the value of its actual length without 0 (this makes
        # sense to me in my wording)
        return len(lst)

    while left < right:
        mid = (left + right) // 2
        if lst[mid] != mid: # if the value of the index of mid is not mid, then the value is incorrect since its sorted
            #so it should be adding up to be its middle counterpart, however, most of the edge cases have been taken
            #care of so this is binary search basically
            right = mid # recursive so it will keep searching and switching the boundaries
        else:
            left = mid + 1 #if you break from this loop, then the recursion will stop and check the other side
    return left # does not matter since the loop will break before left or right equals each other
